...
Renderer.render = function(type,mode,objects,lights,camera){
	gl.clearColor(0.1,0.1,0.1,1);
	gl.clear( gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT );
	Renderer.shader = null;
	//mat4.lookAt(cam.view, cam.eye, cam.center, cam.up);

	if (type == Scene.FORWARD){
		Renderer.forwardRender(mode,objects,lights,camera);
	}else{
		Renderer.deferredRender(mode,objects,lights,camera);
	}
}

Renderer.forwardRender = function(mode,objects,lights,cam){
	...
	for (var i = 0; i < objects.length; i++){
		...
		for (var l = 0; l < lights.length; l++){
		    ...
		    object.objectRenderer.render(mode,uniforms,light.type);
...
Renderer.createGBuffer = function(objects,cam){
		...
		for (var i = 0; i < objects.length ; ++i){
			...
			Renderer.shader = MicroShaderManager.getShader("gbuffer",["deferred_vertex"],["gbuffer_fragment"],"microShaders.xml");
			if (Renderer.shader)
				Renderer.shader.uniforms(uniforms).draw(object.objectRenderer.mesh);
		 }
	});
}

Renderer.deferredRender = function(mode,objects,lights,cam){


	this.createGBuffer(objects,cam);
	...
	if (mode != Scene.FULL){
		...
		for (var l = 0; l < lights.length; l++){
			...
			Renderer.shader = MicroShaderManager.getShader("deferred",["SCREEN_VERTEX_SHADER"],["deferred_fragment"],"microShaders.xml");
			if (Renderer.shader)
				Renderer.shader.toViewport(uniforms);
